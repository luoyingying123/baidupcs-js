...
const bytes = 1024 * 1024 * 1; //文件大小
const chunkSize = 1024 * 1024; //分块下载的大小
const file = './a.bin'; //文件路径
const url_1 = 'https://allall01.baidupcs.com/file/';
const url_2 = 'https://xafj-cm10-1.baidupcs.com/file/';

const limit_1 = pLimit(2);
...
function connection(range) {
    return new Promise((resolve, reject) => {
        const oper = retry.operation();
        oper.attempt((c) => {
            const config = {
                method: 'GET',
                url: ((range / chunkSize) % 2 === 0 ? url_1 : url_2),
                headers: {
                    'User-Agent': '',
